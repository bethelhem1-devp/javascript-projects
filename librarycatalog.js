//this code is about how we retrive library catalogs
...
for (let i = 0; i < catalog.length; i++) {
  ...
  // Check if the year is not "Unknown"
  if (entry.year !== "Unknown") {
    // If the entry's year is smaller than the current oldest, update it
    if (entry.year < oldestYear) {
      oldestYear = entry.year;
    }
    
    // If the entry's year is larger than the current newest, update it
